<header class="bg-white border-b border-dga-gray-200 px-3 sm:px-6 py-2.5 sm:py-4 shadow-sm flex justify-between items-center gap-2 sm:gap-4">
  ...
  <div class="flex items-center gap-2 sm:gap-4 shrink-0">
    @if(config('app.demo_mode') && auth()->user()?->role === 'student')
      @php($demoStudents = \App\QSpark\Support\DemoStudents::all())
      @php($demoStudentId = \App\QSpark\Support\DemoStudents::currentId())
      <div class="hidden md:flex items-center gap-1" title="{{ app()->getLocale() == 'ar' ? 'تبديل الطالب (وضع العرض)' : 'Switch student (demo)' }}">
