                    ...
                    @forelse($users as $user)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('qspark.admin.users.edit', $user->id) }}" class="text-dga-primary-600 hover:text-dga-primary-900 ml-3">{{ __('messages.edit_action') }}</a>
                                @if($user->id !== auth()->id())
                                    <form method="POST" action="{{ route('qspark.admin.users.delete', $user->id) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
