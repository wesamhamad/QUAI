...
const priorityStyles = {
  urgent: { bg: 'bg-error-100 dark:bg-red-900/40', border: 'border-error-500', text: 'text-error-600 dark:text-red-300', label: 'عاجل', labelEn: 'Urgent' },
  important: { bg: 'bg-gold-100 dark:bg-gold-900/40', border: 'border-gold-500', text: 'text-gold-700 dark:text-gold-300', label: 'مهم', labelEn: 'Important' },
  suggestion: { bg: 'bg-info-100 dark:bg-blue-900/40', border: 'border-info-500', text: 'text-info-600 dark:text-blue-300', label: 'اقتراح', labelEn: 'Suggestion' },
};
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="text-sm font-bold text-gray-900 dark:text-white">{lang === 'ar' ? rec.title : rec.titleEn}</h3>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${style.text} font-medium`}>{lang === 'ar' ? style.label : style.labelEn}</span>
                  </div>
                  <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">{lang === 'ar' ? rec.description : rec.descriptionEn}</p>
                </div>
                <button className="shrink-0 bg-white dark:bg-gray-700 text-sa-600 dark:text-sa-400 text-xs font-medium px-3 py-1.5 rounded-lg border border-sa-200 dark:border-sa-700 hover:bg-sa-50 dark:hover:bg-sa-900/30 transition-colors">
                  {lang === 'ar' ? rec.actionLabel : rec.actionLabelEn}
                </button>
              </div>
            </div>
          );
