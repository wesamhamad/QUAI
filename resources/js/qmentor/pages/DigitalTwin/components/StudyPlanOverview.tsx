...
const statusStyles = {
  completed: { bg: 'bg-sa-100 dark:bg-sa-900/50', border: 'border-sa-300 dark:border-sa-700', text: 'text-sa-700 dark:text-sa-300', label: 'مكتمل' },
  'in-progress': { bg: 'bg-gold-100 dark:bg-gold-900/50', border: 'border-gold-300 dark:border-gold-700', text: 'text-gold-700 dark:text-gold-300', label: 'حالياً' },
  remaining: { bg: 'bg-gray-100 dark:bg-gray-700/50', border: 'border-gray-300 dark:border-gray-600', text: 'text-gray-500 dark:text-gray-400', label: 'متبقي' },
  failed: { bg: 'bg-error-100 dark:bg-red-900/50', border: 'border-red-300 dark:border-red-700', text: 'text-error-500 dark:text-red-300', label: 'راسب' },
};
...
function ProgressRing({ percent, size = 100 }: { percent: number; size?: number }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="text-xs text-gray-500 dark:text-gray-400 mt-5">مكتمل</span>
    </div>
  );
}
  ...
  if (studyPlan.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <EmptyState title="لا توجد خطة دراسية" description="لم يتم العثور على خطة دراسية للطالب" icon="book" />
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex-1 grid grid-cols-3 gap-3 text-center">
          ...
          <div className="bg-gold-50 dark:bg-gold-900/30 rounded-xl p-3">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">حالياً</p>
          </div>
      ...
      <div>
        <div className="flex justify-between text-sm mb-1.5">
          <span className="text-gray-600 dark:text-gray-400">تقدم الخطة</span>
      ...
      {/* Prerequisite Tree */}
      <div>
        <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">شجرة المتطلبات</h3>
        <div className="space-y-2">
          {categories.map(cat => {
            ...
            return (
              <div key={cat}>
                <p className="text-xs font-medium text-gray-400 dark:text-gray-500 mb-1.5">{categoryLabels[cat]}</p>
                  ...
                  {courses.map((course, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        title={`${course.name} (${course.prerequisites.length > 0 ? 'يتطلب: ' + course.prerequisites.join(', ') : 'لا متطلبات'})`}
                      >
                        <p className={`text-xs font-bold ${style.text}`}>{course.code}</p>
                        <p className="text-[10px] text-gray-500 dark:text-gray-400 truncate max-w-[90px]">{course.name}</p>
                      </motion.div>
                    );
                  })}
                </div>
      ...
      {suggested.length > 0 && (
        <div>
          <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">مقررات مقترحة للفصل القادم</h3>
          <div className="space-y-2">
            {suggested.map((course, i) => (
              ...
              >
                <div>
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-400 mr-2">{course.name}</span>
                </div>
                <span className="text-xs text-gray-500 dark:text-gray-400">{course.creditHours} ساعات</span>
              </motion.div>
            ))}
          </div>
        </div>
