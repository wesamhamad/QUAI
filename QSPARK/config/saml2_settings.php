...
return $settings = array(
     ...
     *
     * Local: myqulocal -> /saml/myqulocal/login
     * Test:  myqutest  -> /saml/myqutest/login
     * Prod:  myqu      -> /saml/myqu/login
     */
    'idpNames' => ['myqulocal', 'myqutest', 'myqu'],
    ...
    /**
     * Where the SAML ACS handler redirects on success when the SAML response
     * carries no RelayState. Set to /fetch-token so the post-SAML flow goes:
     * SAML ACS -> /fetch-token -> qu-api SSO -> /receive-token -> dashboard.
     */
    'loginRoute' => env('SAML2_LOGIN_URL', '/fetch-token'),
